...
### Generation
- **backend/sdl/generate.py** (289 lines)
...
- **backend/sdl/world.py** (484 lines)
...
### Validation & Testing
- **backend/tests/test_sldb.py** (38 test cases)
...
1. **Wire conversation retrieval**: Make Claude's knowledge graph queries search conversation turns
   - Location: backend/primnox2/knowledge/graph.py
...
# Run full test suite
python -m pytest backend/tests/test_sldb.py -v
```
