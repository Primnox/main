      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-[600px] mx-4 bg-zinc-950 border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-white placeholder-white/25 outline-none text-sm font-mono"
              />
              <div className="flex items-center gap-1 shrink-0">
                <kbd className="px-2 py-0.5 bg-white/5 text-white/30 rounded text-[10px] font-mono border border-white/10">ESC</kbd>
              </div>
            </div>
            ...
            <div ref={listRef} className="max-h-[400px] overflow-y-auto py-2">
              {Object.keys(groups).length === 0 && (
                <div className="px-5 py-8 text-center text-white/30 font-mono text-xs">
                  No commands match "{query}"
                </div>
              )}
              {Object.entries(groups).map(([group, items]) => (
                <div key={group} className="mb-1">
                  <div className="px-5 py-1.5 text-[10px] font-mono uppercase tracking-widest text-white/20">
                    {group}
                  </div>
                  {items.map((item) => {
                    const idx = globalIdx++;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-5 py-3 text-left transition-colors ${
                          isActive ? 'bg-primary/10 text-white' : 'text-white/60 hover:text-white'
                        }`}
                      >
                        <span className={`shrink-0 ${isActive ? 'text-primary' : 'text-white/30'}`}>
                          {item.icon}
                        </span>
              ...
              {query.trim() && onSendMessage && (
                <div className="border-t border-white/5 mt-1 pt-1">
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-5 py-3 text-left transition-colors ${
                      selected === globalIdx ? 'bg-primary/10 text-white' : 'text-white/60 hover:text-white'
                    }`}
                  >
                    <Sparkles size={16} className={selected === globalIdx ? 'text-primary' : 'text-white/30'} />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium">Ask AI: "{query}"</div>
                      <div className="text-[11px] text-white/30">Send as message to Primnox</div>
                    </div>
            ...
            {/* Footer hint */}
            <div className="px-5 py-2.5 border-t border-white/5 flex items-center gap-4 text-[10px] font-mono text-white/20">
              <span className="flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-white/5 rounded border border-white/10">↑↓</kbd> navigate
              </span>
              <span className="flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-white/5 rounded border border-white/10">↵</kbd> select
              </span>
              <span className="flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-white/5 rounded border border-white/10">ESC</kbd> close
              </span>
