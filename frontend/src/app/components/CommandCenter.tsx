    ...
    <div className="flex-1 flex h-full bg-zinc-950 overflow-hidden">
      ...
      <div className={`border-r border-white/5 bg-zinc-900/20 flex flex-col pt-12 text-white overflow-hidden shrink-0 transition-all duration-300 ${showLeftPane ? "w-64 lg:w-72 opacity-100" : "w-0 opacity-0 pointer-events-none"}`}>
        ...
        <div className="px-6 mb-6">
          <button onClick={() => loadChat('current')} className="w-full flex items-center justify-between p-3 bg-primary text-black font-bold rounded-xl hover:bg-white active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50 shadow-[0_0_20px_rgba(79,70,229,0.3)] group">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-3 pb-8 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              {pinnedChats.map((c: any) => (
                <button key={c.id} onClick={() => loadChat(c.id)} className={`w-full text-left p-3 rounded-lg text-sm active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50 group flex flex-col gap-1 ${activeChatId === c.id ? 'bg-primary/20 text-white' : 'hover:bg-white/5 text-white/70'}`}>
          ...
          {/* Folders Section */}
          <div>
            <button onClick={() => setFoldersOpen(!foldersOpen)} className="w-full px-3 py-1 text-[10px] font-mono text-white/40 uppercase tracking-widest mb-2 flex items-center justify-between hover:text-white active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50">
            ...
            {foldersOpen && (
              <div className="space-y-1">
                {chatFolders.map((f: any) => (
                  <button key={f.id} className="w-full text-left p-3 rounded-lg text-sm hover:bg-white/5 text-white/70 active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50 flex items-center justify-between group">
          ...
          {/* History Section */}
          <div>
             <button onClick={() => setHistoryOpen(!historyOpen)} className="w-full px-3 py-1 text-[10px] font-mono text-white/40 uppercase tracking-widest mb-2 flex items-center justify-between hover:text-white active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50">
        ...
        <div className="p-4 border-t border-white/5">
           <button className="w-full flex items-center gap-3 text-white/40 hover:text-white text-xs p-2 rounded hover:bg-white/5 active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50">
      ...
      <div className="flex-1 flex flex-col h-full relative">
          ...
          {liveMessages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-8 duration-1000 mt-20">
              ...
              <div className="flex gap-4">
                <button onClick={() => setInputValue("Analyze my current workspace")} className="bg-white/5 hover:bg-white/10 text-white/60 hover:text-white px-6 py-3 rounded-full font-mono text-[11px] uppercase tracking-widest active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50">Analyze Workspace</button>
                <button onClick={() => setInputValue("Summarize system status")} className="bg-white/5 hover:bg-white/10 text-white/60 hover:text-white px-6 py-3 rounded-full font-mono text-[11px] uppercase tracking-widest active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50">System Status</button>
              </div>
            </div>
          ) : (
        ...
        <footer className="p-8 lg:p-12 pt-0 z-20 pointer-events-auto shrink-0">
          <div className="max-w-4xl mx-auto flex items-center justify-between w-full gap-2">
            <button onClick={() => setShowLeftPane(!showLeftPane)} className="p-3 bg-zinc-900 rounded-2xl hover:bg-zinc-800 text-white/50 hover:text-white shrink-0 active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50" title="Toggle Chat History"><History size={20}/></button>
            ...
            <button 
              ...
              className="p-4 text-white/20 hover:text-primary hover:bg-primary/10 rounded-2xl mr-2 group/btn active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50"
              title="Attach_File"
            >
            ...
            {attachedFile && (
              <div className="flex items-center gap-2 bg-primary/20 text-primary px-3 py-1.5 rounded-xl mr-4 border border-primary/30">
                ...
                <button onClick={removeAttachedFile} className="hover:text-white active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50"><X size={14} /></button>
              </div>
            )}

            <button 
              onClick={handleSend}
              className="bg-primary text-black w-14 h-14 rounded-2xl flex items-center justify-center hover:bg-white shadow-2xl group-hover:shadow-primary/40 disabled:opacity-50 disabled:active:scale-100 active:scale-95 transition-all duration-300 ease-out focus-visible:ring-1 focus-visible:ring-emerald-500/50"
