...
/*
// â”€â”€â”€ AI Custom Block (disabled - not yet integrated into schema) â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Auto-Save Status Type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export const NotesIconSidebar = ({ notes = [], onExport }: { notes: Note[], onExport?: () => void, sendMessage?: (text: string) => void }) => {
  ...
  const activeNote = notes[activeNoteIdx];

  const prevNoteIdxRef = useRef(-1);

  // Sync editor state when switching notes
  useEffect(() => {
    if (activeNote && prevNoteIdxRef.current !== activeNoteIdx) {
      setEditTitle(activeNote.title || "");
      setEditText(activeNote.text || "");
      prevNoteIdxRef.current = activeNoteIdx;
      setShowAskAI(false);
      setAiAnswer("");
      setAiQuery("");
    }
  ...
  useEffect(() => {
    ...
    loadContent();
  }, [activeNoteIdx, editor]); // We trigger this when activeNoteIdx changes

  const onEditorChange = async () => {
    const markdown = await editor.blocksToMarkdownLossy(editor.document);
    setEditText(markdown);
    debouncedSave(editTitle, markdown, activeNoteIdx, activeWorkspace);
  };

  // â”€â”€â”€ Auto-Save with Debounce â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const debouncedSave = useCallback((title: string, text: string, idx: number, project: string) => {
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    saveTimerRef.current = setTimeout(() => {
      persistNote(title, text, idx, project);
    }, 1500);
  }, [persistNote]);
  ...
  const deleteNote = async () => {
    ...
    try {
      ...
      if (activeNoteIdx > 0) {
        setActiveNoteIdx(activeNoteIdx - 1);
      }
    } catch (e) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 's') {
        e.preventDefault();
        if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
        persistNote(editTitle, editText, activeNoteIdx, activeWorkspace);
      }
  ...
  }, [editTitle, editText, activeNoteIdx, persistNote]);

  const onTitleChange = (val: string) => {
    setEditTitle(val);
    debouncedSave(val, editText, activeNoteIdx, activeWorkspace);
  };
  ...
  // â”€â”€â”€ Ask AI Logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const renderNoteTree = (parentId: number | undefined | null = null, depth: number = 0) => {
    ...
    return children.map(note => (
      <div key={note.id || `temp-${note.originalIdx}`} className="w-full">
        <div 
          onClick={() => setActiveNoteIdx(note.originalIdx)}
          className={`px-3 py-1.5 text-sm cursor-pointer rounded flex items-center justify-between group transition-colors ${activeNoteIdx === note.originalIdx ? 'bg-white/10 text-white' : 'text-white/60 hover:bg-white/5 hover:text-white/90'}`}
          style={{ paddingLeft: `${0.75 + depth * 1.5}rem` }}
        >
          <div className="flex items-center gap-2 overflow-hidden">
            <span className="text-base leading-none">{note.icon || 'ðŸ“„'}</span>
  ...
  return (
    <div className="h-full flex flex-col md:flex-row overflow-hidden bg-black text-[#D4D4D4] font-sans">
      ...
      {/* â”€â”€â”€ Main Editor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
        ...
        <div className="h-12 border-b border-white/10 flex items-center px-6 justify-between bg-transparent shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-mono mr-2">
              ...
              {saveStatus === 'saved' && (
                <span className="text-emerald-400/80">âœ“ Saved</span>
              )}
          ...
          {activeNote ? (
            <article className="max-w-3xl w-full px-8 py-16 flex flex-col">
              ...
              <p className="text-[10px] text-white/20 font-mono mb-8">
                {wordCount} words Â· {readingTime} min read Â· {activeNote.timestamp ? new Date(activeNote.timestamp).toLocaleDateString() : 'just now'}
              </p>
