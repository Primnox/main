...
description: building a presentation as an animated HTML deck, or converting a .pptx into one
triggers: html slides, html deck, web deck, animated slides, animated deck, interactive presentation, pitch deck, keynote, beautiful slides, slides for a talk, convert pptx, pptx to html
---

<!--
Vendored from https://github.com/zarazhangrui/frontend-slides (MIT, (c) 2025
Zara Zhang). The licence travels with it in ./LICENSE; keep both together.

Two adaptations for Primnox, and nothing else changed:

  * `triggers:` added — loader.select() matches on it, and without one the skill
    could only ever be reached by the model asking for it by name.
  * `description:` shortened. The loader puts every skill's description in the
    system prompt of EVERY turn, so the original paragraph would have been
    charged to questions that will never make a slide. The full explanation is
    the body, which is only paid for when the skill is actually selected.

Trigger words are deliberately disjoint from `themed-documents`. That skill owns
real .pptx/.docx/.pdf files via python-pptx and reportlab; this one produces HTML
and PDF and CANNOT emit a .pptx. Overlapping triggers would load both bodies into
one turn, which on a local 7B is most of the context window spent before the
question is read.
-->
