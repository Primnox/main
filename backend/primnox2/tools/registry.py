...
def describe_for_prompt(*, exclude_persistent: bool = False,
    ...
    if focus == "desktop":
        ...
        if chosen:
            specs = chosen
    if only:
        # A shortlist from `retrieval.candidates`, which ranks tools against
        # the request. Registry ORDER is preserved rather than ranking order:
        # a list sorted by confidence tells the model which one the retriever
        # liked, and the retriever is right 8 times in 10 — good enough to bias
        # a weak model into agreeing with it, and not good enough to be worth
        # that. The shortlist narrows the field; the model still chooses.
        shortlist = [s for s in specs if s.name in set(only)]
        if shortlist:
            specs = shortlist
