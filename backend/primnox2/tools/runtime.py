...
def _shortlist(request: "str | None") -> "list[str] | None":
    """The tools worth showing for this request, or None for all of them.

    A fourteen-way choice is where a small model loses: measured, it picks the
    right tool about half the time and its mistakes are confusions between
    near-synonyms. An encoder ranks the field first so the model makes a
    five-way choice instead — recall@3 of 10/10 on the retrieval benchmark,
    with no language model involved in the ranking.

    None whenever anything is off: no request, encoder still loading, a machine
    that cannot run it. None means the full catalogue, which is the behaviour
    that existed before this. Losing the shortlist costs prompt size; losing
    the prompt costs the turn.
    """
    if not request:
        return None
    try:
        from . import retrieval
        return retrieval.candidates(request)
    except Exception:                                    # pragma: no cover
        return None
...
def system_prompt(*, incognito: bool = False,
    ...
    # A disambiguation TABLE was tried here and removed. Measured on
    # qwen2.5:0.5b over ten core tasks: descriptions alone score 5/10 on
    # tool choice; adding a table naming three confusable pairs scored
    # 4/10, and trimming it to two pairs still scored 4/10. It cost a
    # point and ~145 tokens of every turn.
    #
    # Worth writing down because the same device WORKS for the desktop
    # tools, and the difference explains when to reach for it. That one
    # teaches an ORDER for tools with no near-synonyms, and adds
    # information the descriptions cannot carry. A table of confusable
    # pairs adds no information — the descriptions already state the
    # distinction — and it raises the SALIENCE of every tool it names.
    # When the failure is over-selecting one of a pair, naming that pair
    # again is the wrong lever: `recall_memory` appeared in the table and
    # promptly started winning `remember` and `graph_query` tasks too.
...
def parse_call(text: str) -> dict | None:
    ...
    if not matches:
        # Last resort, and only when the BODY is complete — see `_unterminated`.
        opener = _unterminated().search(text)
        if opener is not None and _looks_complete(opener.group(2)):
            return _parse_body(opener.group(1), opener.group(2))
    if not matches:
        return None
...
def _looks_complete(body: str) -> bool:
    """Whether a tool block's body is FINISHED, judged structurally.

    `_parse_body` cannot answer this. It is deliberately lenient — for a tool
    with one string parameter it accepts the body as-is, because JSON is a
    hostile envelope for source code — so it happily turns the truncated
    `{"code": "print(1)"` into a runnable call. That leniency is right where
    it lives and useless as a completeness test.

    So the question is asked of the shape instead. A model that finished
    either closed its JSON or closed its fence; a model that was cut off did
    neither. Anything else is refused, which keeps the default on the side of
    the existing behaviour: a turn that says nothing fails with a retryable
    `empty_reply` rather than running half a call.
    """
    body = body.strip()
    if not body:
        return False
    if body.startswith("{"):
        ...
        return True
    if body.startswith("```"):
        return body.count("```") >= 2 and body.endswith("```")
    return False


def _unterminated() -> re.Pattern:
    """A block that opened correctly and never closed.

    Measured on Qwen3.5-0.8B: asked to multiply two numbers it replied with
    exactly the form this prompt teaches — `<tool name="run_python">` and a
    fenced python block — and simply stopped, 92 characters, no `</tool>`.
    Three of its ten answers failed that way, each of them naming the right
    tool and carrying runnable code. Same failure the two variants above
    already exist for: the model called the tool correctly and was refused
    over its punctuation.

    Tried LAST, and only when nothing well-formed matched, so a reply that
    closes its tag properly never reaches this. The opener carries a
    REGISTERED tool name, and prose does not produce `<tool name="run_python">`
    by accident.

    THE BODY MUST PARSE. That is the line between this and the case
    `test_an_unfinished_tool_block_fails_rather_than_replying_blank` exists to
    protect, and the two look alike until you read the body:

        <tool name="run_python">{"code": "print(1)"      truncated mid-JSON
        <tool name="run_python">```python…```            complete, tag missing

    The first is a model that was cut off mid-generation, and running half a
    call is how you execute the beginning of something much larger. It must
    keep failing with a retryable `empty_reply`. The second is a model that
    finished its thought and forgot the punctuation. So the caller runs the
    body through `_parse_body` and accepts this only when nothing came back
    malformed — the completeness of the body, not the presence of the tag, is
    what says the model was done.

    A bare code fence with no tool tag stays inert either way, which is the
    other distinction that matters — `format_result` promises the user that a
    fence written in a reply is shown rather than run.
    """
    global _UNTERMINATED
    names = tool_names()
    if _UNTERMINATED is None or names != _UNTERMINATED_NAMES:
        alternation = "|".join(re.escape(n) for n in names)
        _UNTERMINATED = re.compile(
            rf'<tool\s+name="({alternation})"\s*>(.*)$', re.S)
        _UNTERMINATED_NAMES[:] = names
    return _UNTERMINATED


_UNTERMINATED: "re.Pattern | None" = None
_UNTERMINATED_NAMES: list[str] = []
