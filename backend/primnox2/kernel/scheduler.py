...
class Scheduler:
    ...
    def _run_chat(self, job: dict) -> None:
        ...
        preamble = "\n\n".join(
            # `request=` is deliberately NOT passed. Narrowing the catalogue
            # by embedding similarity is implemented (tools/retrieval.py) and
            # measured WORSE: 4/10 against 5/10 on qwen2.5:0.5b, despite the
            # retriever's own recall@3 being 10/10. See that module for why —
            # it is a genuine result about retrieval, not a bug.
