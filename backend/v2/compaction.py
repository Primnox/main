...
def summarize_region(messages: list[dict], *, budget_tokens: int = DEFAULT_SUMMARY_TOKENS) -> tuple[str, list[str]]:
    ...
    if estimate_tokens(text) > budget_tokens:
        # Trim until it actually fits, measured with the estimator in use,
        # rather than cutting at a fixed 4-chars-per-token guess and trusting
        # the arithmetic. That guess tracked V1's estimator; V2's reads about
        # 14% higher, so the same cut produced a 143-token "capped" summary
        # against a 140-token budget — a cap that quietly was not one. The
        # suffix counts toward the budget too, which the old form also missed.
        suffix = "\n…[summary truncated]"
        keep = budget_tokens * 4
        while keep > 0:
