...
import { type PermissionRequest } from '../lib/crs';
...
/* A permission question. Auto-approved ones are still shown — the user should
   ...
   Desktop resolves against the loopback backend. On web the resolution is a
   sealed `permission.resolved` event, so the caller supplies it — the same
   injection TurnBlock uses for retry. Without a handler the options render but
   do not pretend to be live. */
export function PermissionBlock({
  p,
  onResolve,
}: {
  p: PermissionRequest;
  onResolve?: (permissionId: string, optionId: string) => void;
}) {
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            {p.options.map(o => (
              <button key={o.id} onClick={() => onResolve?.(p.id, o.id)} disabled={!onResolve}
                className="px-2.5 py-1 rounded-lg border border-on-surface/15 hover:bg-on-surface/[0.06] transition duration-150 px-label disabled:opacity-50">
                {o.label}
              </button>
            ))}
          </div>
