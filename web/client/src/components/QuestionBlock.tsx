...
import { type UserQuestion } from '../lib/crs';
 ...
 *
 * Desktop POSTs the answer to the loopback backend. On web the answer is a
 * sealed event the caller owns, so it arrives as `onAnswer` — the same
...
export function QuestionBlock({
  ...
  q: UserQuestion;
  onAnswer?: (questionId: string, choice: string) => void | Promise<void>;
}) {
        ...
        <div className="min-w-0 flex-1">
          ...
          {q.answered ? (
            ...
            <div role="group" aria-label="Answer"
              ...
              {q.options.map(o => (
                <button key={o.id} onClick={() => void answer(o.id)}
                  disabled={sending !== null || !onAnswer}
