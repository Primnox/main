...
#
# `parents[4]` only resolves in the dev tree — four directories up from
# storage/vault.py lands at the repo root, then into website/wordlist.txt.
# Inside a frozen build there is no repo root: __file__ resolves somewhere
# under PyInstaller's extraction temp dir, and that fixed parent count no
# longer lands anywhere real. Same shape of bug privacy/mirror.py's
# _resolve_model_source() already exists to avoid for the PII model —
# resolved the same way, checking bundled locations before the dev-tree path.
def _wordlist_path() -> Path:
    ...
    candidates.append(Path(__file__).resolve().parents[4] / "website" / "wordlist.txt")
    ...
    raise FileNotFoundError(
        "BIP-39 wordlist not found in any known location: "
